...
int main(void) {
    ...
    uint8_t version_low = (uint8_t)(version & 0xFFu);
    unsigned ver_major = (unsigned)((version_low >> 4) & 0x0Fu);
    unsigned ver_minor = (unsigned)(version_low & 0x0Fu);
    ...
    proto_header_bits_t view = {
        ...
        .length = length,
        .flags = flags,
        .reserved = (unsigned)((raw.flags_raw >> 5) & 0x07u),
    };
